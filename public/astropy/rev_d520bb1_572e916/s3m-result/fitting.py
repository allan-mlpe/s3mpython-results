...
__all__ = ['LinearLSQFitter', 'NonLinearLSQFitter', 'SLSQPFitter',           'JointFitter', 'Fitter']
...
class  Fitter (object) :
	...
	def _validate_constraints(self, model):
        message = '{0} cannot handle {{0}} constraints.'.format(                self.__class__.__name__)
...
>>>>>>> YOURS
        ...
        if (any([tuple(b) != (None, None) for b in model.bounds.values()]) and                'bounds' not in self.supported_constraints):
        ...
        if (model.ineqcons and                'ineqcons' not in self.supported_constraints):
	...
	def _model_to_fit_params(self):
        ...
        if any(self.model.fixed.values()) or any(self.model.tied.values()):
            ...
            return (self.model.parameters.copy(), fitparam_indices,                    param_indices)
...
<<<<<<< MINE
        ...
        if len(farg) == 2:
            x, y = farg
            if y.ndim == 2:
                assert y.shape[1] == model_copy.param_dim, (                    "Number of data sets (Y array is expected to equal "                    "the number of parameter sets")
        ...
        if not multiple and model_copy.param_dim > 1:
            raise ValueError("Attempting to fit a 1D data set to a model "                             "with multiple parameter sets")
        if rcond is None:
...
=======
        ...
        if len(farg) == 2:
            ...
            if y.ndim == 2:
                ...
                x = self._map_domain_window(model_copy, x)
            if any(model_copy.fixed.values()):
                lhs = self._deriv_with_constraints(model_copy,                                                   fitparam_indices,                                                   x=x)
            else:
                lhs = model_copy.deriv(x=x)
            if len(y.shape) == 2:
                rhs = y
            ...
            if any(model_copy.fixed.values()):
                lhs = self._deriv_with_constraints(model_copy,                                                   fitparam_indices, x=x, y=y)
            else:
                lhs = model_copy.deriv(x=x, y=y)
            if len(z.shape) == 3:
                rhs = np.array([i.flatten() for i in z]).T
...
class  NonLinearLSQFitter (Fitter) :
    ...
    def __init__(self):
        ...
        self.fit_info = {'nfev': None,                         'fvec': None,                         'fjac': None,                         'ipvt': None,                         'qtf': None,                         'message': None,                         'ierr': None,                         'status': None}
...
>>>>>>> YOURS
	...
	def __call__(self, model, x, y, z=None, weights=None,                 maxiter=DEFAULT_MAXITER,                 epsilon=DEFAULT_EPS, estimate_jacobian=False):
        """
        Fit data to this model.
        ...
        if model_copy.param_dim != 1:
                        raise ValueError("NonLinearLSQFitter can only fit one "                             "data set at a time")
        if model_copy.deriv is None or estimate_jacobian:
            dfunc = None
        else:
            dfunc = self._wrap_deriv
        init_values, _ = model_copy._model_to_fit_params()
        fitparams, status, dinfo, mess, ierr = optimize.leastsq(            self.errorfunc, init_values, args=farg, Dfun=dfunc,            col_deriv=model_copy.col_deriv, maxfev=maxiter, epsfcn=epsilon,            full_output=True)
...
>>>>>>> YOURS
    ...
    ModelLinearityError
        A linear model is passed to a nonlinear fitter
    """
	def __call__(self, model, x, y, z=None, weights=None, maxiter=MAXITER,                 epsilon=EPS, estimate_jacobian=False):
        """
        Fit data to this model.
...
class  SLSQPFitter (Fitter) :
    ...
    def __init__(self):
        ...
        self.fit_info = {            'final_func_val': None,            'numiter': None,            'exit_mode': None,            'message': None        }
	def errorfunc(self, fps, *args):
        """
...
>>>>>>> YOURS
	...
	def __call__(self, model, x, y, z=None, weights=None, verblevel=0,                 maxiter=DEFAULT_MAXITER, epsilon=DEFAULT_EPS):
        """
        Fit data to this model.
        ...
        fitparams, final_func_val, numiter, exit_mode, mess = \
            optimize.fmin_slsqp(            self.errorfunc, p0, args=farg, disp=verblevel, full_output=1,            bounds=bounds, eqcons=eqcons, ieqcons=ineqcons, iter=maxiter,            acc=1.E-6, epsilon=DEFAULT_EPS)
    ...
    .. [1] http://www.netlib.org/toms/733
    """
	def __call__(self, model, x, y, z=None, weights=None, verblevel=0,                 maxiter=MAXITER, epsilon=EPS):
        """
        Fit data to this model.
        ...
        ineqcons = np.array(model_copy.ineqcons) 
        fitparams, final_func_val, numiter, exit_mode, mess = \
            optimize.fmin_slsqp(            self.errorfunc, p0, args=farg, disp=verblevel, full_output=1,            bounds=bounds, eqcons=eqcons, ieqcons=ineqcons, iter=maxiter,            acc=1.E-6, epsilon=EPS)
        model_copy._fitter_to_model_params(fitparams)
        self.fit_info['final_func_val'] = final_func_val
        self.fit_info['numiter'] = numiter
        self.fit_info['exit_mode'] = exit_mode
...
class  JointFitter (object) :
	...
	def __call__(self, *args):
        ...
        self.fitparams[:], _ = optimize.leastsq(self.errorfunc, self.fitparams,                                              args=args)
