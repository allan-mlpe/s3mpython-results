...
=======
__all__ = ['sigma_clip', 'binom_conf_interval', 'binned_binom_proportion','signal_to_noise_oir_ccd']
>>>>>>> YOURS
...
def binom_conf_interval(k, n, conf=0.68269, interval='wilson'):
    ...
    **3. The Wald Interval.** This interval is given by
    ...
    if interval == 'wilson' or interval == 'wald':
        ...
        kappa = np.sqrt(2.) * min(erfinv(conf), 1.e10)  
        k = k.astype(np.float)
        n = n.astype(np.float)
        p = k / n
...
def sigma_clip(data, sig=3, iters=1, cenfunc=np.median, varfunc=np.var,               axis=None, copy=True):
